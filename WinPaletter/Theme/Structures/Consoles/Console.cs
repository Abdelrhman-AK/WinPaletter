...
namespace WinPaletter.Theme.Structures
{
    ...
    public class Console : ManagerBase<Console>
    {
        ...
        /// <summary>
        /// Create console structure with default data and registry keys to be used for loading/saving data
        /// </summary>
        /// <param name="regKeys"></param>
        public Console(string Signature, string[] regKeys) { this.signature = Signature; this.regKeys = regKeys; }
        ...
        public void Load(Console @default)
        {
            ...
            {
                string RegAddress = $@"HKEY_CURRENT_USER\Console{(string.IsNullOrEmpty(key) ? string.Empty : $@"\{key}")}";
        ...
        public void Save(string consoleType, TreeView treeView = null)
        {
        ...
        public void Save(string consoleType, bool overwriteHKU, TreeView treeView = null)
        {
            Program.Log?.Write(LogEventLevel.Information, $"Applying {consoleType} preferences...");
            ...
            if (overwriteHKU)
            {
                Save_Internal(@"HKEY_USERS\.DEFAULT", treeView);
            }
        }

        /// <summary>
        /// Process console structure data into registry
        /// </summary>
        ...
        private void Save_Internal(string scopeReg, TreeView treeView = null)
        {
            ...
            if (Enabled)
            {
                ...
                {
                    string RegAddress = $@"{scopeReg}\Console{(string.IsNullOrEmpty(key) ? string.Empty : $@"\{key}")}";

                    try
                    {
                        if (scopeReg.ToUpper() == "HKEY_CURRENT_USER")
                        {
                            if (!string.IsNullOrEmpty(key)) Registry.CurrentUser.CreateSubKey($@"Console\{key}", true).Close();
                        }
                    }
                    ...
                    WriteReg(treeView, RegAddress, "ColorTable00", Color.FromArgb(0, ColorTable00.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable01", Color.FromArgb(0, ColorTable01.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable02", Color.FromArgb(0, ColorTable02.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable03", Color.FromArgb(0, ColorTable03.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable04", Color.FromArgb(0, ColorTable04.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable05", Color.FromArgb(0, ColorTable05.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable06", Color.FromArgb(0, ColorTable06.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable07", Color.FromArgb(0, ColorTable07.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable08", Color.FromArgb(0, ColorTable08.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable09", Color.FromArgb(0, ColorTable09.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable10", Color.FromArgb(0, ColorTable10.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable11", Color.FromArgb(0, ColorTable11.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable12", Color.FromArgb(0, ColorTable12.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable13", Color.FromArgb(0, ColorTable13.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable14", Color.FromArgb(0, ColorTable14.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "ColorTable15", Color.FromArgb(0, ColorTable15.Reverse()).ToArgb());
                    WriteReg(treeView, RegAddress, "PopupColors", Convert.ToInt32($"{PopupBackground:X}{PopupForeground:X}", 16));
                    WriteReg(treeView, RegAddress, "ScreenColors", Convert.ToInt32($"{ScreenColorsBackground:X}{ScreenColorsForeground:X}", 16));
                    WriteReg(treeView, RegAddress, "CursorSize", CursorSize);
                    ...
                    {
                        WriteReg(treeView, RegAddress, "FaceName", FaceName, RegistryValueKind.String);
                        WriteReg(treeView, RegAddress, "FontFamily", 54);
                    }

                    WriteReg(treeView, RegAddress, "FontSize", (PixelHeight << 16) | (PixelWidth & 0xFFFF));
                    WriteReg(treeView, RegAddress, "FontWeight", FontWeight);

                    if (OS.W10_1909)
                    {
                        WriteReg(treeView, RegAddress, "CursorColor", Color.FromArgb(0, W10_1909_CursorColor.Reverse()).ToArgb());
                        WriteReg(treeView, RegAddress, "CursorType", W10_1909_CursorType);
                        WriteReg(treeView, RegAddress, "WindowAlpha", W10_1909_WindowAlpha);
                        WriteReg(treeView, RegAddress, "ForceV2", W10_1909_ForceV2 ? 1 : 0);
                        WriteReg(treeView, RegAddress, "LineSelection", W10_1909_LineSelection ? 1 : 0);
                        WriteReg(treeView, RegAddress, "TerminalScrolling", W10_1909_TerminalScrolling ? 1 : 0);
                    }
